...
class Inky(GameComponent):
    ...
    def dentro_casa_fantasmas(self, elementos_fase, posicao_futura=None, direcao_atual=None):
        ...
        if posicao_futura is None and direcao_atual is None:
            ...
            if direcao_atual == direcao.cima:
                return Rect((self.movimento.posicao[x], self.movimento.posicao[y]-1), (16, 14)).colliderect(elementos_fase.casa_fantasmas)

            elif direcao_atual == direcao.baixo:
                return Rect((self.movimento.posicao[x], self.movimento.posicao[y]+1), (16, 14)).colliderect(elementos_fase.casa_fantasmas)

            elif direcao_atual == direcao.esquerda:
                return Rect((self.movimento.posicao[x]-1, self.movimento.posicao[y]), (16, 14)).colliderect(elementos_fase.casa_fantasmas)
    ...
    def interseccao(self, elementos_fase, acao, direcao_atual=None, posicao=None):
        if posicao is None or direcao_atual is None:
            if acao["direcao"] in [direcao.esquerda, direcao.direita]:
                return not self.colisao(direcao.cima, elementos_fase, acao["posicao"]) or \
                       not self.colisao(direcao.baixo, elementos_fase, acao["posicao"])

            elif acao["direcao"] in [direcao.cima, direcao.baixo]:
                return not self.colisao(direcao.esquerda, elementos_fase, acao["posicao"]) or \
                       not self.colisao(direcao.direita, elementos_fase, acao["posicao"])

        else:
            if direcao_atual in [direcao.esquerda, direcao.direita]:
                return not self.colisao(direcao.cima, elementos_fase, posicao) or \
                       not self.colisao(direcao.baixo, elementos_fase, posicao)

            elif direcao_atual in [direcao.cima, direcao.baixo]:
                return not self.colisao(direcao.esquerda, elementos_fase, posicao) or \
                       not self.colisao(direcao.direita, elementos_fase, posicao)
